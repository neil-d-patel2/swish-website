...
## 2026-08-17 — `pnpm run build` fails with TS2345/TS2322 on a brand-new route (`"/privacy"` not assignable)

**Error:**
```
src/routes/privacy.tsx(5,38): error TS2345: Argument of type '"/privacy"' is not assignable to parameter of type 'keyof FileRoutesByPath | undefined'.
src/routes/contact.tsx(338,19): error TS2322: Type '"/privacy"' is not assignable to type '"/" | "." | "/stores" | ... '.
```
(one TS2322 per file containing a `<Link to="/privacy">`)

**Root cause:**
The build script is `tsc -b && vite build`. `src/routeTree.gen.ts` — the file that defines the `FileRoutesByPath` union of valid route paths — is generated by the TanStack Router **Vite plugin**, so it is only regenerated during `vite build` (or `vite dev`). On the first build after adding a new file in `src/routes/`, `tsc` runs *before* the generator has ever seen the new file, so the new path doesn't exist in the union yet and every reference to it fails to typecheck. Nothing was actually wrong with the new route.

**Fix:**
Run `npx vite build` (or start the dev server) once to regenerate `src/routeTree.gen.ts`, then run the normal `pnpm run build`, which now passes.
